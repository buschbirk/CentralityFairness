    ...
    * `_get_current_jobs` (scheduler command to return jobid, status, jobname one job per line)
    ...
    def submit(self): 
        ...
        job = template_str.format(walltime=self.walltime, 
                                  ...
                                  cores_per_executor=int(self.cores_per_executor),
...
def start_cluster(memory, cores_per_executor=1, timeout=30, spark_home=None):
    ...
    print("master_command: " + master_command)
    if scheduler=='slurm':
    ...
    # slaves_command = slaves_command.replace("desktop1", "172.16.16.101")

    logger.info('slaves command: ' + slaves_command)
    print("SLAVE COMMANDS: {}".format(slaves_command))
    print("Executing")

    logger.info('XXX SLAVES SLAVES: ' + slaves_command)

    p = subprocess.Popen(slaves_command, env = env, shell=True)
    p.wait()

    logger.info("XXXX Finished slave commands")
